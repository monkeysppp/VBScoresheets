...
function createWindow () {
  ...
  module.exports.internal.mainWindow.loadURL('file://' + path.join(__dirname, '/app/index.html'))
...
function indexReadyListener (event) {
  return files.listTeamFiles()
    .then((filelist) => {
      debug('#indexReadyListener got file list')
      if (filelist.length === 0) {
        event.sender.send('change-state', 'loading', 'add-first-team')
        debug('#indexReadyListener drawing addFirstTeam')
      } else {
        event.sender.send('change-state', 'loading', 'pick-a-team')
        debug('#indexReadyListener drawing pickATeam')
      }
    })
    .catch((err) => {
